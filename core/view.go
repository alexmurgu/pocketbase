	...
	return app.RunInTransaction(func(txApp App) error {
		...
		for i := len(dependentViews) - 1; i >= 0; i-- {
			if err := txApp.DeleteView(dependentViews[i].Name); err != nil {
				return fmt.Errorf("failed to drop dependent view %q while saving view %q: %w", dependentViews[i].Name, dangerousViewName, err)
			}
		}
...
func findDependentViews(app App, tableOrViewName string) ([]viewDef, error) {
	rows, err := loadViewDependencies(app)
	if err != nil {
		return nil, err
	}

	nodes, graph := buildViewDependencyGraph(rows)
...
func findAllViewsInDependencyOrder(app App) ([]viewDef, error) {
	...
	views := make([]viewDef, 0, len(sorted))
...
func loadViewDependencies(app App) ([]viewDependency, error) {
	const query = `
		select
			u.view_name,
	...
	var rows []viewDependency
	if err := app.DB().NewQuery(query).All(&rows); err != nil && !errors.Is(err, sql.ErrNoRows) {
		return nil, err
	}
...
func buildViewDependencyGraph(rows []viewDependency) (map[string]viewDef, map[string][]string) {
